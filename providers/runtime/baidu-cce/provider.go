// Package cce is the D2-readiness RuntimeProvider against Baidu CCE. Every
// request flows through an injected Doer so tests run on fixtures and never
// touch the wire; signing of live requests lands with the cloud milestone.
package cce
...
// Config carries region and credentials; credentials are expected to arrive
// from a Secret source, never hardcoded.
type Config struct {
	Region    string
	Endpoint  string
	AccessKey string
...
func (c Config) endpoint() string {
	...
	return strings.Replace("https://cce.{region}.baidubce.com", "{region}", c.Region, 1)
}
...
// Provider adapts Baidu CCE to the platform runtime contract.
...
// GetCluster returns the lifecycle state of one cluster.
func (p *Provider) GetCluster(ctx context.Context, clusterID string) (string, error) {
	body, err := p.call(ctx, http.MethodGet, apiPrefix+"/cluster/"+clusterID, nil)
	if err != nil {
		return "", err
	}
	var out struct {
	...
	if jErr := json.Unmarshal(body, &out); jErr != nil {
		return "", fmt.Errorf("decode cluster state: %w", jErr)
	}
	return out.State, nil
}

// CreateNodePool adds a worker pool to an existing cluster.
func (p *Provider) CreateNodePool(ctx context.Context, clusterID, poolName string, replicas int) error {
	_, err := p.call(ctx, http.MethodPost, apiPrefix+"/cluster/"+clusterID+"/nodepool",
		map[string]any{"name": poolName, "replicas": replicas})
	return err
}

// DeleteCluster tears a cluster down.
func (p *Provider) DeleteCluster(ctx context.Context, clusterID string) error {
	_, err := p.call(ctx, http.MethodDelete, apiPrefix+"/cluster/"+clusterID, nil)
	return err
}

// InstallGateway registers the ingress gateway addon on the cluster.
func (p *Provider) InstallGateway(ctx context.Context, clusterID string) error {
	_, err := p.call(ctx, http.MethodPost, apiPrefix+"/cluster/"+clusterID+"/gateway", nil)
	return err
}

// BindWorkload is a D2 wiring point; the MVP shared runtime needs no CCE op.
...
// Healthz probes the CCE control plane reachability.
func (p *Provider) Healthz(ctx context.Context) error {
	if _, err := p.call(ctx, http.MethodGet, apiPrefix+"/cluster", nil); err != nil {
...
func init() {
	types.Runtimes.Register("baidu-cce", func() types.RuntimeProvider {
