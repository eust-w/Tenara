...
// apiPrefix scopes every TKE REST call made by Tenara.
const apiPrefix = "/tke"
...
func (c Config) endpoint() string {
	...
	return strings.Replace("https://tke.{region}.tencentcloudapi.com", "{region}", c.Region, 1)
}
...
// Provider adapts Tencent TKE to the platform runtime contract.
...
func (p *Provider) call(ctx context.Context, method, path string, body any) ([]byte, error) {
	...
	if status >= 400 {
		return nil, fmt.Errorf("%w: Tencent TKE %s %s returned %d",
...
// Healthz probes the Tencent TKE control plane reachability.
...
func init() {
	types.Runtimes.Register("tencent-tke", func() types.RuntimeProvider {
