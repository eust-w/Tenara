...
console.log("[1] remote protocol cases");
{
	...
	check("remote tools/list matches catalog", JSON.stringify(names) === JSON.stringify(EXPECTED_NAMES));
...
console.log("[3] parity between transports");
{
	...
	const stdioNames = getText()
		...
		.flat()
		.sort();
	check("tool-set diff empty across transports", JSON.stringify(remoteNames) === JSON.stringify(stdioNames));
}
