...
// BFF login: exchange credentials with the control plane and translate the
// bearer token into an httpOnly session cookie. No auth bypass is introduced.
export async function POST(request: Request) {
  ...
  const payload = (await upstream.json().catch(() => ({}))) as { token?: unknown; error?: string };
